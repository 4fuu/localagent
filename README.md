...
LocalAgent 不是一个把模型直接接进聊天框的轻量 Bot，而是一个面向**本地个人助手场景**的 Agent Runtime。

它的核心设计理念是：**把 Agent 放进一套可控的工程化运行时里**，而不是只依赖单次 Prompt。具体体现在：

- **Runtime First**：先提供 Hub、Gateway、IndexService、RuntimeBroker 等基础设施，再让 Agent 在其上运行
- **职责分层**：Main Agent 负责规划、记忆和调度，Task Agent 负责执行具体任务
- **能力外置**：能力来自模型 + 工具 + 记忆 + 技能 + 沙箱，而不是把所有逻辑塞进提示词
- **边界清晰**：通过 workspace 作用域、权限控制、容器沙箱和 secrets 管理约束副作用

如果借用业界相近的说法，可以把它理解为一个更偏**本地助手 / 个人自动化**方向的 agent harness；区别在于，本项目的重点不是软件交付流水线，而是让个人 Agent 在本地环境中安全、可扩展、可编排地持续工作。
